...
class Neuralnet:
    ...
    public NeuralNet(int[] NETWORK_LAYER_SIZE){
        ...
        for (int index=0
            ...
            this.bias[index] = buildRandomArray(
                NETWORK_LAYER_SIZE[index], Categoriser.BIAS_RANGE_SMALLEST, Categoriser.BIAS_RANGE_BIGGEST)
            // exclude the input layer[0]
            if (index > 0){
                // weights for a layer, specific from previous layer.
                weights[index] = buildRandomArray(NETWORK_LAYER_SIZE[index], NETWORK_LAYER_SIZE[index-1],
                                                  Categoriser.WEIGHTS_RANGE_SMALLEST, Categoriser.WEIGHTS_RANGE_BIGGEST)
            }
        }
    }
    ...
    public void train(Dataset set, int loops, int batch_size){
        ...
        for (int index=0
            ...
            for (int b=0
                 b < batch_size
                 b++){
                this.training(batch.getInput(b), batch.getOutput(
                    b), Categoriser.LEARNING_RATE)
            }
        }
    }
    ...
    public double[] calculateOutput(double[] input){
        ...
        for (int layer=1
             ...
             layer + +){
            for (int neuron=0
                 neuron < NETWORK_LAYER_SIZE[layer]
                 neuron++){
                double sum = 0
                for (int previousNeuron=0
                     previousNeuron < NETWORK_LAYER_SIZE[layer-1]
                     previousNeuron++){
    ...
    public void backpropError(double[] target){
        ...
        for (int layer=NETWORK_SIZE-2
            ...
            for (int neuron=0
                 ...
                 neuron++){
                double sum = 0
                for (int nextNeuron=0
                     nextNeuron < NETWORK_LAYER_SIZE[layer+1]
                     nextNeuron++){
                    sum += weights[layer+1][nextNeuron][neuron] *
                    error_signal[layer+1][nextNeuron]
                }
                this.error_signal[layer][neuron] = sum *
                output_derivative[layer][neuron]
            }
        }
    }
    ...
    def meanSquareErr(l_input, target_output):
        ...
        for i in range(len(target_output)):
            err += (target_output[i] - self.output[self.NETWORK_SIZE-1]
                    [i]) * target_output[i] - self.output[NETWORK_SIZE-1][i]

        return err / (1.0 * len(target_output))

    def meanSquareErrFunction(dataset):
        err = 0
        for i in range(len(dataset)):
            err += meanSquareErr(dataset.getInput(i), dataset.getOutput(i))
        return err / (1.0 * len(dataset))

    def sigmoidFuntion(n):
